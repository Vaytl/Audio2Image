...
/// <summary>
/// AudioSet ontology: 527 sound event classes used by PANNs CNN14.
/// Labels are ordered by index (0-526) matching the model's clipwise_output tensor.
/// Source: https://github.com/qiuqiangkong/audioset_tagging_cnn
/// </summary>
public static class AudioSetLabels
{
    /// <summary>All 527 AudioSet class display names, indexed by model output position.</summary>
    ...
    [
        ...
        "Whimper", "Wail, moan", "Sigh", "Singing", "Choir", "Yodeling",
        ...
        "Growling", "Whimper (dog)", "Cat", "Purr", "Meow", "Hiss",
        "Caterwaul", "Livestock, farm animals, working animals", "Horse",
        "Clip-clop", "Neigh, whinny", "Cattle, bovinae", "Moo", "Cowbell",
        "Pig", "Oink", "Goat", "Bleat", "Sheep", "Fowl",
        "Chicken, rooster", "Cluck", "Crowing, cock-a-doodle-doo", "Turkey",
        ...
        "Banjo", "Sitar", "Mandolin", "Zither", "Ukulele",
        ...
        "Swing music", "Bluegrass", "Funk", "Folk music",
        ...
        "Tick-tock", "Gears", "Pulleys", "Sewing machine", "Mechanical fan",
        "Air conditioning", "Cash register", "Printer", "Camera",
        "Single-lens reflex camera", "Tools", "Hammer", "Jackhammer",
        ...
        "Burst, pop", "Eruption", "Boom", "Wood", "Chop", "Splinter",
    ...
    /// <summary>
    /// Get top-N predicted labels with their probabilities from model output (527-dim sigmoid).
    /// </summary>
    /// <param name="probabilities">527-element array of sigmoid probabilities from clipwise_output.</param>
    /// <param name="topN">Number of top labels to return.</param>
    /// <param name="threshold">Minimum probability to include (default 0.1).</param>
    public static List<(string Label, float Probability)> GetTopLabels(
        float[] probabilities, int topN = 5, float threshold = 0.1f)
    {
        if (probabilities.Length != Labels.Length)
            throw new ArgumentException(
                $"Expected {Labels.Length} probabilities, got {probabilities.Length}.");

        var indexed = new (float Prob, int Index)[probabilities.Length];
        for (int i = 0; i < probabilities.Length; i++)
            indexed[i] = (probabilities[i], i);

        // Sort descending by probability
        Array.Sort(indexed, (a, b) => b.Prob.CompareTo(a.Prob));

        var result = new List<(string, float)>(topN);
        for (int i = 0; i < Math.Min(topN, indexed.Length); i++)
        {
            if (indexed[i].Prob < threshold) break;
            result.Add((Labels[indexed[i].Index], indexed[i].Prob));
        }

        return result;
    }

    /// <summary>Total number of AudioSet classes.</summary>
    public static int NumClasses => Labels.Length;
}
